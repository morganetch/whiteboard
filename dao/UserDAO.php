...
class UserDAO extends DAO {
	...
	public function searchUsers($search, $ownId, $boardMakerId,$board_id){
		$sql = "SELECT * FROM `wb_users` WHERE `username` LIKE :username AND `id` != :ownId AND `id` != :boardMakerId AND `id` NOT IN (SELECT `user_id` FROM `wb_invites` WHERE board_id = :board_id) LIMIT 10";
		$stmt = $this->pdo->prepare($sql);
		$stmt->bindValue(':username', '%' . $search . '%');
		$stmt->bindValue(":ownId", $ownId);
		$stmt->bindValue(":board_id", $board_id);
		$stmt->bindValue(":boardMakerId", $boardMakerId);
		$stmt->execute();
		return $stmt->fetchAll(PDO::FETCH_ASSOC);
	}
